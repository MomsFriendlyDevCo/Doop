<component name="usersList">
module.exports = {
	route: '/users',
	data() { return {
		users: undefined,
		query: {
			sort: 'email',
		},
	}},
	methods: {
		refresh() {
			return Promise.resolve()
				.then(()=> this.$loader.start('users', !this.users))
				.then(()=> this.$http.get('/api/users', {
					params: {
						...this.query,
					},
				}))
				.then(res => this.users = res.data.map(user => ({
					...user,
					url: `/users/${user._id}`,
				})))
				.catch(this.$toast.catch)
				.finally(()=> this.$loader.stop('users'))
		},
	},
	created() {
		return this.refresh();
	},
};
</component>

<template name="usersList">
	<div v-if="users" class="card">
		<div class="btn-group-float">
			<a v-href="'/users/invite'" class="btn btn-icon btn-circle btn-success fa fa-envelope" v-tooltip="'Invite a new user to this project'"></a>
		</div>
		<div v-if="!users.length" class="alert alert-warning">No users found</div>
		<table v-else class="table table-hover">
			<thead>
				<tr>
					<th class="col-status"/>
					<th>Email</th>
					<th>Name</th>
				</tr>
			</thead>
			<tbody>
				<tr v-for="user in users" v-href="{url: user.url, transition: 'slide-right'}">
					<td class="col-status">
						<i v-if="user.status == 'active'" class="fas fa-circle text-success" v-tooltip="'User is active'"/>
						<i v-else-if="user.status == 'deleted'" class="fas fa-circle text-warning" v-tooltip="'User has been deleted'"/>
						<i v-else class="fas fa-question-circle text-warning" v-tooltip="'User status is unknown'"/>
					</td>
					<td>
						{{user.email}}
					</td>
					<td>
						{{user.name}}
					</td>
				</tr>
			</tbody>
		</table>
	</div>
</template>

<component name="usersEdit">
module.exports = {
	route: '/users/:id',
	data() { return {
		user: undefined,
		spec: undefined,
	}},
	methods: {
		refresh() {
			return Promise.resolve()
				.then(()=> this.$loader.start('usersEdit'))
				.then(()=> Promise.all([
					this.$http.get(`/api/users/${this.$route.params.id}`)
						.then(res => this.user = res.data),
					this.$http.get('/api/users/meta')
						.then(res => this.spec = res.data),
				]))
				.then(()=> this.$sitemap.setTitle(this.user.name))
				.catch(this.$toast.catch)
				.finally(()=> this.$loader.stop('usersEdit'))
		},
		save(notification = false, redirect = false) {
			return Promise.resolve()
				.then(()=> this.$loader.startBackground('usersEditSave'))
				.then(()=> this.$http.post(`/api/users/${this.$route.params.id}`, this.user))
				.then(()=> notification && this.$toast.success('user edits saved'))
				.then(()=> redirect && this.$router.push('/users'))
				.catch(this.$toast.catch)
				.finally(()=> this.$loader.stop('usersEditSave'))
		},
	},
	created() {
		return this.refresh();
	},
};
</component>

<template name="usersEdit">
	<form v-if="user" class="form-horizontal" @submit.prevent="save(true, false)">
		<div class="btn-group-float">
			<a @click="save(true, true)" class="btn btn-icon btn-lg btn-circle btn-success fa fa-check" v-tooltip="'Save user edits'"></a>
		</div>

		<!-- Card: User information {{{ -->
		<div class="card">
			<div class="card-header">User information</div>
			<div class="card-body">
				<div class="form-group row">
					<label class="col-4 col-form-label">Name</label>
					<div class="col-8 col-form-label">
						<input type="text" v-model="user.name" class="form-control"/>
					</div>
				</div>
			</div>
		</div>
		<!-- }}} -->

		<!-- Card: Permissions {{{ -->
		<div class="card" fixme="Should only be visible if user has project.members[].usersPromote">
			<div class="card-header">Permissions</div>
			<div class="card-body">
				<permissions
					:selected="user.permissions"
					:spec="spec"
					spec-prefix="permissions."
				/>
			</div>
		</div>
		<!-- }}} -->

	</form>
</template>
